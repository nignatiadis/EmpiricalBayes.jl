...
# Diff to base v1, finer grid
# also store many Efron results for multiple dfs

nreps = 1
k_max = 6 #in total 25*16 replications
ks_par = 1:100#1:10
...
for k=1:k_max
    ...
    for l = 1:nreps
        ...
        brad_fit =  [fit(SmoothedBradDeconvolveR, Xs, 0.05; c0=0.01,prior_grid = collect(-3.6:0.05:3.6),
                           pDegree = p) for p in brad_degs]
        f_nb = fit(BinnedMarginalDensityNeighborhood, Xs_train, ds; nboot=1000)
    ...
    @save "/scratch/users/ignat/sims/base_sim_Jan10_v2/mysim_$(i)_$(k)_$(k_rep).jld" res
end
